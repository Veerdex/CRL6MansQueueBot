// One-off export: same density-based skew mechanism as simulate-mmr-skew.mjs (Attempt 3), but at
// a smaller scale — 96 players, 10 rounds — instead of the usual 996/100 baseline, with skew
// factor fixed at 0.5 (dampens the negative side).
//
// Run with: node scripts/export-mmr-skew-csv-96p10r.mjs [outputPath]
...
const outputPath = process.argv[2] || "mmr-skew-96p-10r.csv";

const players = runSimulation(SKEW_FACTOR);
...
console.log(`Wrote ${lines.length - 1} rows (${NUM_PLAYERS} players, ${ROUNDS} rounds, skew_factor=${SKEW_FACTOR}) to ${outputPath}`);
