...
export interface HallOfFameSeason {
  ...
  seasonNumber: number;
  isActive: boolean;
  slots: HallOfFameSlot[]; // always length 5: rank1, rank2, rank3, games, streak
}

function toHofPlayer(player: PlayerRow): HallOfFamePlayer {
  return {
    id: player.id,
...
function buildSlots(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    slots.push({
      ...
      player: entry ? toHofPlayer(entry.player) : null,
      mmr: entry ? entry.mmr : 0,
    });
  }
  slots.push({
    kind: "games",
    player: gamesWinner ? toHofPlayer(gamesWinner.player) : null,
    gamesPlayed: gamesWinner ? gamesWinner.value : 0,
  });
  slots.push({
    kind: "streak",
    player: streakWinner ? toHofPlayer(streakWinner.player) : null,
...
// standing (gold/silver/bronze), 4th = most games played that season (both queues, matching
// season_history's own definition), 5th = highest Rank Queue win streak reached that season.
// Closed seasons read the archival `season_history` record (seasonClose.ts's permanent
// standings snapshot) for the top-3/games-played slots; the still-active season has no
// season_history rows yet, so all 5 slots are computed live off current data instead — which is
// also exactly why that section renders "greyed out / unconfirmed" on the page, since it can
// still change as the season progresses. Neither path filters `is_test_data`, matching the
// established precedent for read-side leaderboard-style queries (getAllPlayersWithGames,
// getMatchHistory, getHeadToHeadData) — see CLAUDE.md, "Match History".
  ...
  return ordered.map((season) => {
    if (season.is_active) {
      ...
      return {
        ...
        isActive: true,
        slots: buildSlots(top3, gamesWinner, streakWinner),
      };
    }
